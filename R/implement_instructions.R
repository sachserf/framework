...
implement_instructions <-
    function(cache_dir = '.cache')
    {
        ...
        if (file.exists(
            ...
            stop(
                "Required files in cache are missing. Recall function prepare_instructions() and try again."
            )
        }
        # reload instructions
        ...
        image_cache <-
            paste0(file.path(cache_dir, filename_noxt, basename(filename_noxt)),
                   '.RData')
        ...
        filename_out <-
            gsub(pattern = source_dir,
                 replacement = "",
                 filename_noxt)
        filename_out <- gsub(pattern = "/", replacement = "_", filename_out)
        figure_out <-
            file.path(target_dir_figure, paste0(row_names, filename_out))
        ...
        df_source_files$instruction_ext[which(df_source_files$file_ext == "R")] <-
            "source"
        df_source_files$instruction_ext[which(df_source_files$file_ext == "Rmd")] <-
            "render"
        df_source_files$instruction_ext[which(df_source_files$file_ext == "md")] <-
            "render"
        df_source_files$instruction_ext[which(df_source_files$file_ext == "Rnw")] <-
            "knit"
        ...
        if (any(df_source_files$file_ext != "R")) {
            df_source_files[which(df_source_files$file_ext != "R"), "use_spin"] <-
                NA
        }
        ...
        if (length(which(df_source_files$use_spin == TRUE)) > 0) {
            df_source_files[which(df_source_files$use_spin == TRUE), "instruction_no_cache"] <-
                "render"
        }
        ...
        if (file.exists(file.path(cache_dir, 'df_source_files_temp.rds'))) {
            unlink(
                x = file.path(cache_dir, 'df_source_files_temp.rds'),
                recursive = TRUE
            )
        }
